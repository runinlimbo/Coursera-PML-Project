...
This project explores various models that predict the manner in which test subjects perform a dumbbell bicep curl exercise.  Three models were tested on the provided data.  The best performing model of those reviewed, based on level of accuracy, was the random forest model (**98.37% accurate on the validation set**).  The chosen model was then run on the 20 test cases to predict the technique used in each case.
...
Based on overall accuracy results, the Random Forest Model holds the greatest accuracy (**99.37%**) on the validation dataset.  The **out of sample error estimate** is equivalent to 1 minus the accuracy of the RF model on the validation set, which is equal to **0.63%**.  Overall Accuracy Results for each model is below: 
```{r include = FALSE}
library(knitr)
library(kableExtra)
```
